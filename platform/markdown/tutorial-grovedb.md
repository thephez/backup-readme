...
Create a new folder called “grovedb-tutorials”. In “grovedb-tutorials”, create another new folder called “storage”. We will use “storage” to store the data from the tutorials. This is where the actual GroveDB instance and data will reside. If you want to rerun any of the tutorials which insert values, since overwriting isn’t allowed, you must delete the contents of “storage” before running again, otherwise you’ll get an error.

## Adding GroveDB to Cargo.toml

GroveDB can be added to your project’s Cargo.toml file like so:

``` toml
[dependencies]
grovedb = { git = "https://github.com/dashevo/grovedb.git" }
```

# Tutorial 1 - Open

GroveDB uses the `open()` function to [open](https://github.com/dashpay/grovedb/blob/009787b79538dae833ed6711253852046dfcc59d/grovedb/src/lib.rs) an existing instance of GroveDB at the given path in your filesystem, unless there is no existing instance, in which case a new one will be created with an empty root tree. The only argument for `open()` is the filesystem path.

In “grovedb-tutorials” create a new Rust project using `cargo new open`. Navigate to the new project, add GroveDB to the Cargo.toml file, and paste the following code into the main.rs file. Do `cargo run`.
...
Now, if you open the “storage” folder, you’ll see it’s populated with a bunch of files. GroveDB is set up and ready for us to insert some data.
